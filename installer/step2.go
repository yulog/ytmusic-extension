...
func (t *Step2) SetModel(model *Model) {
	t.model = model
}

func (t *Step2) Build(context *guigui.Context, appender *guigui.ChildWidgetAppender) error {

	t.buttonPrev.SetText("Back")
	t.buttonPrev.SetOnUp(func() {
		t.model.SetStep(step1)
	})
	t.buttonBack.SetText("Next")
	t.buttonBack.SetOnUp(func() {
		t.model.SetStep(step3)
	})

	t.form.SetItems([]basicwidget.FormItem{
		{
			PrimaryWidget:   &t.buttonPrev,
			SecondaryWidget: &t.buttonBack,
		},
	})
